...
input_raw_number = 55                            # Raw number from a csv file with model parameters

path_for_csv_inputs = 'resources/inputs.csv'    # Path to csv file with model parameters
path_for_json_outputs = 'outputs/01'            # Path to HB JSON outputs directory
...
for raw in inputs_lib:
  if inputs_lib.line_num < input_raw_number:
    continue
  elif inputs_lib.line_num > input_raw_number:
    break
  cur_inputs = raw
...
if cur_inputs != None:
  width, length, height, rotation, north_wwr, west_wwr, south_wwr, east_wwr, north_shade_size, north_svf, west_shade_size, west_svf, south_shade_size, south_svf, east_shade_size, east_svf, north_adiabatic, west_adiabatic, south_adiabatic, east_adiabatic, insulation, glazing_type = float(cur_inputs['width']), float(cur_inputs['length']), float(cur_inputs['height']), float(cur_inputs['rotation']), float(cur_inputs['north_wwr']), float(cur_inputs['west_wwr']), float(cur_inputs['south_wwr']), float(cur_inputs['east_wwr']), float(cur_inputs['north_shade_size']), float(cur_inputs['north_svf']), float(cur_inputs['west_shade_size']), float(cur_inputs['west_svf']), float(cur_inputs['south_shade_size']), float(cur_inputs['south_svf']), float(cur_inputs['east_shade_size']), float(cur_inputs['east_svf']), float(cur_inputs['north_adiabatic']), float(cur_inputs['west_adiabatic']), float(cur_inputs['south_adiabatic']), float(cur_inputs['east_adiabatic']), float(cur_inputs['insulation']), float(cur_inputs['glazing_type'])
  inp_file_name = path_for_csv_inputs.replace("/", "_")
  inp_file_name = inp_file_name.replace(".", "_")
  model_name = "model_from_" + inp_file_name + "_raw_" + str(input_raw_number)
...
if context_count > 0:
    ...
    hb_model = Model(identifier="model", rooms=[hb_zone], orphaned_shades=context)
else: hb_model = Model(identifier="model", rooms=[hb_zone])
...
### MODEL OUTPUT ###
hb_model.to_hbjson(name=model_name,folder=path_for_json_outputs)